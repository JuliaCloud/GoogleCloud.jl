...
Base.values(store::KeyStore{K, V}) where {K, V} = (x for x in (get(store, key, Nothing) for key in keys(store)) if x !== Nothing)
...
IteratorSize(::Type{KeyStore{K, V}}) where {K, V} = Base.SizeUnknown()

# notifications
function watch(store::KeyStore{K, V}, channel_id::AbstractString, address::AbstractString) where {K, V}
